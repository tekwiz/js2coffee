...
(function() {
var Node, Typenames, Types, UnsupportedError, exports, narcissus, parser, tokens, _;
...
this.thenPart.walk(options, fn, this);
}
if (this.elsePart != null) {
this.elsePart.walk(options, fn, this);
}
if (this.cases) {
...
(function() {
var Builders, Code, Node, Transformers, Typenames, Types, UnsupportedError, blockTrim, body, build, buildCoffee, exports, isSingleLine, ltrim, p, paren, parser, re, rtrim, strEscape, strRepeat, transform, trim, unreserve, unshift, _, _ref, _ref2;
var __slice = Array.prototype.slice, __bind = function(fn, me){ return function(){ return fn.apply(me, arguments); }; };
parser = (this.Narcissus || require('./narcissus_packed')).parser;
_ = this._ || require('underscore');
_ref = this.NodeExt || require('./node_ext'), Types = _ref.Types, Typenames = _ref.Typenames, Node = _ref.Node, UnsupportedError = _ref.UnsupportedError;
...
c = new Code;
_.each(this.functions, function(item) {
return c.add(build(item));
});
_.each(this.nonfunctions, function(item) {
return c.add(build(item));
});
return c.toString();
},
...
'identifier': function() {
return unreserve(this.value.toString());
},
'number': function() {
return "" + (this.src());
},
...
'id_param': function() {
var _ref3;
if ((_ref3 = this.toString()) === 'undefined') {
return "" + this + "_";
} else {
return re('id', this);
}
...
return "";
} else if (this.expression.typeName() === 'object_init') {
src = re('object_init', this.expression);
...
'in': function() {
return re('binary_operator', this, 'of');
},
'<<': function() {
return re('binary_operator', this, '<<');
},
...
var left, right;
left = build(this.left());
right = build(this.right());
if (this.isThis && this.isPrototype) {
return "@::";
} else if (this.isThis) {
return "@" + right;
} else if (this.isPrototype) {
return "" + left + "::";
} else if (this.left().isPrototype) {
return "" + left + right;
} else {
return "" + left + "." + right;
}
...
var c, keyword;
c = new Code;
keyword = this.positive ? "while" : "until";
c.add("" + keyword + " " + (build(this.condition)));
c.scope(body(this.body));
return c;
},
'do': function() {
...
var body_, c, keyword;
c = new Code;
keyword = this.positive ? "if" : "unless";
body_ = body(this.thenPart);
if (isSingleLine(body_) && !(this.elsePart != null)) {
c.add("" + (trim(body_)) + "  " + keyword + " " + (build(this.condition)) + "\n");
} else {
c.add("" + keyword + " " + (build(this.condition)));
...
return "" + (re('property_identifier', this.left())) + ": " + (build(this.right()));
},
'object_init': function(options) {
var c, list;
if (options == null) {
options = {};
}
...
this.functions = [];
this.nonfunctions = [];
_.each(this.children, __bind(function(item) {
if (item.isA('function')) {
return this.functions.push(item);
} else {
return this.nonfunctions.push(item);
}
}, this));
last = null;
return _.each(this.nonfunctions, __bind(function(item) {
var expr;
...
'.': function() {
this.isThis = this.left().isA('this');
return this.isPrototype = this.right().isA('identifier') && this.right().value === 'prototype';
},
';': function() {
if (this.expression != null) {
this.expression.parenthesized = false;
if (this.expression.isA('call')) {
return this.expression.type = Typenames['call_statement'];
}
}
},
'function': function() {
return this.body.walk({
last: true
}, function(parent, node) {
if (node.isA('return') && node.value) {
return parent.children[parent.children.length - 1] = node.value;
}
});
},
'switch': function() {
return _.each(this.cases, __bind(function(item) {
var block, ch;
...
'block': function() {
return Transformers.script.apply(this);
},
'if': function() {
return Transformers.inversible.apply(this);
},
'while': function() {
return Transformers.inversible.apply(this);
},
'inversible': function() {
if (this.condition.isA('!=')) {
this.condition.type = Typenames['=='];
return this.positive = false;
} else if (this.condition.isA('!')) {
this.condition = this.condition.left();
return this.positive = false;
} else {
...
'==': function() {
if (this.right().isA('null', 'void')) {
this.type = Typenames['existence_check'];
