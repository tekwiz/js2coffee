...
Tokenizer.prototype={get done(){return this.peek(true)===END;},get token(){return this.tokens[this.tokenIndex];},match:function(tt,scanOperand){return this.get(scanOperand)===tt||this.unget();},mustMatch:function(tt){if(!this.match(tt)){throw this.newSyntaxError("Missing "+
...
return tt;},peekOnSameLine:function(scanOperand){this.scanNewlines=true;var tt=this.peek(scanOperand);this.scanNewlines=false;return tt;},skip:function(){var input=this.source;for(;;){var ch=input[this.cursor++];var next=input[this.cursor];if(ch==='\n'&&!this.scanNewlines){this.lineno++;}else if(ch==='/'&&next==='*'){this.cursor++;for(;;){ch=input[this.cursor++];if(ch===undefined)
throw this.newSyntaxError("Unterminated comment");if(ch==='*'){next=input[this.cursor];if(next==='/'){this.cursor++;break;}}else if(ch==='\n'){this.lineno++;}}}else if(ch==='/'&&next==='/'){this.cursor++;for(;;){ch=input[this.cursor++];if(ch===undefined)
return;if(ch==='\n'){this.lineno++;break;}}}else if(ch!==' '&&ch!=='\t'){this.cursor--;return;}}},lexExponent:function(){var input=this.source;var next=input[this.cursor];if(next==='e'||next==='E'){this.cursor++;ch=input[this.cursor++];if(ch==='+'||ch==='-')
...
throw e;s+="\n"+more;}}}
return{parse:parse,parseStdin:parseStdin,Node:Node,DECLARED_FORM:DECLARED_FORM,EXPRESSED_FORM:EXPRESSED_FORM,STATEMENT_FORM:STATEMENT_FORM,Tokenizer:lexer.Tokenizer,FunctionDefinition:FunctionDefinition};}());var exports={definitions:Narcissus.definitions,lexer:Narcissus.lexer,parser:Narcissus.parser};if(typeof module!='undefined'){module.exports=exports;};(function(){var Code,Tokens,Types,UnsupportedError,body,build,exports,getTokens,narcissus,p,parser,re,strEscape,strRepeat,tokens,trim,unreserve,_;var __slice=Array.prototype.slice;if(typeof window!=="undefined"&&window!==null){narcissus=window.Narcissus;_=window._;}else{narcissus=require('./narcissus_packed');_=require('underscore');}
tokens=narcissus.definitions.tokens;parser=narcissus.parser;parser.Node.prototype.left=function(){return this.children[0];};parser.Node.prototype.right=function(){return this.children[1];};parser.Node.prototype.unsupported=function(msg){throw new UnsupportedError("Unsupported: "+msg,this);};parser.Node.prototype.typeName=function(){return Types[this.type];};build=function(item,opts){var name,out;if(opts==null){opts={};}
name='other';if(item!==void 0&&item.typeName){name=item.typeName();}
out=(Tokens[name]||Tokens.other).apply(item,[opts]);if(item.parenthesized!=null){return"("+out+")";}else{return out;}};body=function(item,opts){var str;if(opts==null){opts={};}
str=trim(build(item,opts));if(str.length>0){return str;}else{return"";}};getTokens=function(){var dict,i;dict={};for(i in tokens){if(typeof tokens[i]==='number'){dict[tokens[i]]=i.toLowerCase();}}
return dict;};strRepeat=function(str,times){var i;return((function(){var _results;_results=[];for(i=0;0<=times?i<times:i>times;0<=times?i++:i--){_results.push(str);}
return _results;})()).join('');};re=function(){var args,str,type;type=arguments[0],str=arguments[1],args=3<=arguments.length?__slice.call(arguments,2):[];return Tokens[type].apply(str,args);};trim=function(str){return(""+str).replace(/^\s*|\s*$/g,'');};strEscape=function(str){return JSON.stringify(""+str);};Types=getTokens();unreserve=function(str){var _ref;if((_ref=""+str)==='in'||_ref==='loop'||_ref==='off'||_ref==='on'||_ref==='when'||_ref==='not'||_ref==='until'||_ref==='__bind'||_ref==='__indexOf'){return""+str+"_";}else{return""+str;}};Tokens={'script':function(opts){var c,len;if(opts==null){opts={};}
c=new Code;len=this.children.length;if(len>0){if(opts.returnable!=null){this.children[len-1].last=true;}
if((opts.noBreak!=null)&&this.children[len-1].typeName()==='break'){delete this.children[len-1];}}
if(this.children!=null){_.each(this.children,function(item){if(item.typeName()==='function'){return c.add(build(item));}});_.each(this.children,function(item){if(item.typeName()!=='function'){return c.add(build(item));}});}
return c.toString();},'identifier':function(){return unreserve(this.value);},'number':function(){return""+this.value;},'id':function(){return unreserve(this);},'id_param':function(){var _ref;if((_ref=this.toString())==='undefined'){return""+this+"_";}else{return re('id',this);}},'return':function(){if(!(this.value!=null)){return"return";}else if(this.last!=null){return build(this.value);}else{return"return "+(build(this.value));}},';':function(){if(!(this.expression!=null)){return"";}else if(this.expression.typeName()==='call'){return re('call_statement',this.expression)+"\n";}else{return build(this.expression)+"\n";}},'new':function(){return"new "+(build(this.left()));},'new_with_args':function(){return"new "+(build(this.left()))+"("+(build(this.right()))+")";},'unary_plus':function(){return"+"+(build(this.left()));},'unary_minus':function(){return"-"+(build(this.left()));},'this':function(){return'this';},'null':function(){return'null';},'true':function(){return'true';},'false':function(){return'false';},'void':function(){return'undefined';},'break':function(){return"break\n";},'continue':function(){return"continue\n";},'!':function(){return"not "+(build(this.left()));},'++':function(){return""+(build(this.left()))+"++";},'--':function(){return""+(build(this.left()))+"--";},'~':function(){return"~"+(build(this.left()));},'typeof':function(){return"typeof "+(build(this.left()));},'index':function(){return""+(build(this.left()))+"["+(build(this.right()))+"]";},'throw':function(){return"throw "+(build(this.exception));},'+':function(){return re('binary_operator',this,'+');},'-':function(){return re('binary_operator',this,'-');},'*':function(){return re('binary_operator',this,'*');},'/':function(){return re('binary_operator',this,'/');},'%':function(){return re('binary_operator',this,'%');},'>':function(){return re('binary_operator',this,'>');},'<':function(){return re('binary_operator',this,'<');},'&':function(){return re('binary_operator',this,'&');},'|':function(){return re('binary_operator',this,'|');},'^':function(){return re('binary_operator',this,'^');},'&&':function(){return re('binary_operator',this,'and');},'||':function(){return re('binary_operator',this,'or');},'in':function(){return re('binary_operator',this,'in');},'==':function(){return re('binary_operator',this,'==');},'<<':function(){return re('binary_operator',this,'<<');},'<=':function(){return re('binary_operator',this,'<=');},'>>':function(){return re('binary_operator',this,'>>');},'>=':function(){return re('binary_operator',this,'>=');},'!=':function(){return re('binary_operator',this,'!=');},'===':function(){return re('binary_operator',this,'is');},'!==':function(){return re('binary_operator',this,'isnt');},'instanceof':function(){return re('binary_operator',this,'instanceof');},'=':function(){var sign;sign=this.assignOp!=null?Types[this.assignOp]+'=':'=';return""+(build(this.left()))+" "+sign+" "+(build(this.right()));},',':function(){var list;list=_.map(this.children,function(item){return build(item)+"\n";});return list.join('');},'regexp':function(){var begins_with,flag,m,value;m=this.value.toString().match(/^\/(.*)\/([a-z]?)/);value=m[1];flag=m[2];begins_with=value[0];if(begins_with===' '||begins_with==='='){if(flag.length>0){return"RegExp("+(strEscape(value))+", \""+flag+"\")";}else{return"RegExp("+(strEscape(value))+")";}}else{return"/"+value+"/"+flag;}},'string':function(){return strEscape(this.value);},'call':function(){if(this.right().children.length===0){return""+(build(this.left()))+"()";}else{return""+(build(this.left()))+"("+(build(this.right()))+")";}},'call_statement':function(){if(this.right().children.length===0){return""+(build(this.left()))+"()";}else{return""+(build(this.left()))+" "+(build(this.right()));}},'list':function(){var list;list=_.map(this.children,function(item){return build(item);});return list.join(", ");},'delete':function(){var ids;ids=_.map(this.children,function(el){return build(el);});ids=ids.join(', ');return"delete "+ids+"\n";},'binary_operator':function(sign){return""+(build(this.left()))+" "+sign+" "+(build(this.right()));},'.':function(){if(this.left().typeName()==='this'){return"@"+(build(this.right()));}else{return""+(build(this.left()))+"."+(build(this.right()));}},'try':function(){var c;c=new Code;c.add('try');c.scope(body(this.tryBlock));_.each(this.catchClauses,function(clause){return c.add(build(clause));});if(this.finallyBlock!=null){c.add("finally");c.scope(body(this.finallyBlock));}
return c;},'catch':function(){var c;c=new Code;if(this.varName!=null){c.add("catch "+this.varName);}else{c.add('catch');}
c.scope(body(this.block));return c;},'?':function(){return"(if "+(build(this.left()))+" then "+(build(this.children[1]))+" else "+(build(this.children[2]))+")";},'for':function(){var c;c=new Code;if(this.setup!=null){c.add(""+(build(this.setup))+"\n");}
if(this.condition!=null){c.add("while "+(build(this.condition))+"\n");}else{c.add("while true");}
c.scope(body(this.body));if(this.update!=null){c.scope(body(this.update));}
return c;},'for_in':function(){var c;c=new Code;c.add("for "+(build(this.iterator))+" of "+(build(this.object)));c.scope(body(this.body));return c;},'while':function(){var c;c=new Code;c.add("while "+(build(this.condition)));c.scope(body(this.body));return c;},'do':function(){var c;c=new Code;c.add("while true");c.scope(body(this.body));if(this.condition!=null){c.scope("break unless "+(build(this.condition)));}
return c;},'if':function(){var c;c=new Code;c.add("if "+(build(this.condition)));c.scope(body(this.thenPart));if(this.elsePart!=null){if(this.elsePart.typeName()==='if'){c.add("else "+(build(this.elsePart).toString()));}else{c.add("else\n");c.scope(body(this.elsePart));}}
return c;},'switch':function(){var c;c=new Code;c.add("switch "+(build(this.discriminant))+"\n");_.each(this.cases,function(item){var first;if(item.value==='default'){c.scope("else");}else{c.scope("when "+(build(item.caseLabel))+"\n");}
c.scope(body(item.statements,{noBreak:true}),2);return first=false;});return c;},'array_init':function(){var list;list=re('list',this);return"[ "+list+" ]";},'property_init':function(){return""+(build(this.left()))+": "+(build(this.right()));},'object_init':function(){var c,list;if(this.children.length===0){return"{}";}else if(this.children.length===1){return build(this.children[0]);}else{list=_.map(this.children,function(item){return build(item);});c=new Code;c.scope(list.join("\n"));return c;}},'function':function(){var c,params;c=new Code;params=_.map(this.params,function(str){return re('id_param',str);});if(this.name){c.add(""+this.name+" = ");}
if(this.params.length>0){c.add("("+(params.join(', '))+") ->");}else{c.add("->");}
c.scope(body(this.body,{returnable:true}));return c;},'var':function(){var list;list=_.map(this.children,function(item){if(item.initializer!=null){return""+item.value+" = "+(build(item.initializer));}});return _.compact(list).join("\n")+"\n";},'other':function(){return this.unsupported(""+(this.typeName())+" is not supported yet");},'getter':function(){return this.unsupported("getter syntax is not supported; use __defineGetter__");},'setter':function(){return this.unsupported("setter syntax is not supported; use __defineSetter__");},'label':function(){return this.unsupported("labels are not supported by CoffeeScript");},'const':function(){return this.unsupported("consts are not supported by CoffeeScript");}};Tokens.block=Tokens.script;UnsupportedError=(function(){function UnsupportedError(str,src){this.message=str;this.cursor=src.start;this.line=src.lineno;this.source=src.tokenizer.source;}
UnsupportedError.prototype.toString=function(){return this.message;};return UnsupportedError;})();Code=(function(){function Code(){this.code='';}
Code.prototype.add=function(str){this.code+=str.toString();return this;};Code.prototype.scope=function(str,level){var indent;if(level==null){level=1;}
indent=strRepeat("  ",level);this.code=this.code.replace(/\s*$/,'')+"\n";this.code+=indent+trim(str).replace(/\n/g,"\n"+indent)+"\n";return this;};Code.prototype.toString=function(){return this.code;};return Code;})();p=function(str){if(str.tokenizer!=null){delete str.tokenizer;}
console.log(str);return'';};exports={version:'0.0.3',build:function(str){return trim(build(parser.parse(""+str+"\n")));},UnsupportedError:UnsupportedError};if(typeof window!=="undefined"&&window!==null){window.Js2coffee=exports;}
if(typeof module!=="undefined"&&module!==null){module.exports=exports;}}).call(this);