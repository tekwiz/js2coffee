...
(function() {
  var Code, Tokens, Types, UnsupportedError, body, build, exports, getTokens, narcissus, p, parser, re, strEscape, strRepeat, tokens, trim, unreserve, _;
  ...
  parser = narcissus.parser;
  parser.Node.prototype.left = function() {
    return this.children[0];
  };
  parser.Node.prototype.right = function() {
    return this.children[1];
  };
  parser.Node.prototype.unsupported = function(msg) {
    throw new UnsupportedError("Unsupported: " + msg, this);
  };
  parser.Node.prototype.typeName = function() {
    return Types[this.type];
  };
  build = function(item, opts) {
    var name, out;
    if (opts == null) {
      opts = {};
    }
    name = 'other';
    if (item !== void 0 && item.typeName) {
      name = item.typeName();
    }
    out = (Tokens[name] || Tokens.other).apply(item, [opts]);
    if (item.parenthesized != null) {
      return "(" + out + ")";
    } else {
      return out;
    }
  ...
  re = function() {
    ...
    return Tokens[type].apply(str, args);
  };
  trim = function(str) {
  ...
  Code = (function() {
    ...
    Code.prototype.scope = function(str, level) {
      ...
      this.code += indent + trim(str).replace(/\n/g, "\n" + indent) + "\n";
      return this;
    };
  ...
  exports = {
    build: function(str) {
      return trim(build(parser.parse(str)));
    },
