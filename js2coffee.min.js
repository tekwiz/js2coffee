...
c.scope(body(this.block));return c;},'?':function(){return"(if "+(build(this.left()))+" then "+(build(this.children[1]))+" else "+(build(this.children[2]))+")";},'label':function(){throw"Not supported yo";},'for':function(){var c;c=new Code;if(this.setup!=null){c.add(""+(build(this.setup))+"\n");}
...
console.log(str);return'';};exports={build:function(str){return trim(build(parser.parse(str)));},UnsupportedError:UnsupportedError};if(typeof module==='undefined'){this.Js2coffee=exports;}else{module.exports=exports;}}).call(this);